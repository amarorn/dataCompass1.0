...
export class MongoMessageRepository {
   ...
   */
  async addToHistory(messageData: Omit<MessageDocument, '_id'>): Promise<string> {
    try {
   ...
   */
  async getHistory(): Promise<MessageDocument[]> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('message_history');
   ...
   */
  async getReceivedMessages(): Promise<MessageDocument[]> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('message_history');
   ...
   */
  async getSentMessages(): Promise<MessageDocument[]> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('message_history');
   ...
   */
  async getUserMessages(phoneNumber: string): Promise<MessageDocument[]> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('message_history');
