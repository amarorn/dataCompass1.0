...
export class MongoCSVRepository {
   ...
   */
  async saveProcessedCSV(csvData: Omit<ProcessedCSVDocument, '_id'>): Promise<string> {
    try {
   ...
   */
  async getAllProcessedCSVs(from?: string): Promise<ProcessedCSVDocument[]> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('processed_csvs');
   ...
   */
  async getProcessedCSVById(id: string): Promise<ProcessedCSVDocument | null> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('processed_csvs');
   ...
   */
  async updateCSVAnalysis(id: string, analysis: any): Promise<boolean> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('processed_csvs');
   ...
   */
  async deleteProcessedCSV(id: string): Promise<boolean> {
    try {
      await this.connection.connect();
      const db = this.connection.getDb();
      const collection = db.collection('processed_csvs');
