...
exports.MongoRawRepository = void 0;
...
class MongoRawRepository {
     ...
     */
    async saveRawData(messageId, filename, from, csvData) {
        try {
            ...
            const rawRecords = csvData.map((row, index) => {
     ...
     */
    async getRawDataByMessageId(messageId) {
        try {
            await this.connection.connect();
            const db = this.connection.getDb();
            const collection = db.collection('raw');
     ...
     */
    async saveExploratoryAnalysis(analysis) {
        try {
            await this.connection.connect();
            const db = this.connection.getDb();
     ...
     */
    async getExploratoryAnalysisByMessageId(messageId) {
        try {
            await this.connection.connect();
            const db = this.connection.getDb();
            const collection = db.collection('exploratory_analysis');
     ...
     */
    async getAllExploratoryAnalyses() {
        try {
            await this.connection.connect();
            const db = this.connection.getDb();
            const collection = db.collection('exploratory_analysis');
     ...
     */
    async deleteRawDataByMessageId(messageId) {
        try {
            await this.connection.connect();
            const db = this.connection.getDb();
    ...
    async getRawDataStats() {
        try {
            ...
            const db = this.connection.getDb();
            const collection = db.collection('raw');
            const [totalRecords, messageStats, dateStats, senderStats] = await Promise.all([
                // Total de registros
            ...
            return {
                ...
                topSenders: senderStats.map((stat) => ({
                    from: stat.from,
     ...
     */
    calculateDataQuality(row, dataTypes) {
...
exports.MongoRawRepository = MongoRawRepository;
//# sourceMappingURL=MongoRawRepository.js.map